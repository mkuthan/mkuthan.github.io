...
date: 2017-11-02
tags: [Stream Processing, Apache Kafka, Scala, Performance]
header:
...
What's unique, the only dependency to run Kafka Streams application is a running Kafka cluster.
...
we don't expect duplicates more than 10 seconds between each other.
...
I didn't find another way to deduplicate events with DSL, please let me know if better implementation exists.
...
logging into Kafka can't be disabled using DSL.
You can't also get rid of the window for "this" side of the join (window for events), more about it later on. 
...
And I didn't even count traffic from internal topics replication and standby replicas 
...
The processed page views don't even need to be forwarded to downstream.
...
* I didn't present any Kafka Streams test (what's the shame - I'm sorry) 
...
e.g. Scala compiler couldn't infer KStream generic types. 
