...
* Dataflow recognized late data and hold watermark for one of the input stream.
  Reported data freshness is now ~40-50 seconds.
* Moreover, the watermark is propagated downstream. 
  The window might be finalized only if data from both streams has been processed. 
...
It does not make any sense to allocate resources for the data received from the up-to-date sensors 
if the results will be only calculated when late data finally arrive.
The late data drives the throughput and latency of the whole job.
