...
## Complete results from the experiment (not finished)
...
1. **Phase 1** initial back-fill
    ...
    * The backlog was consumed almost immediately, try to figure out why the delay is decreasing steadily :) 
2. **Phase 2** both streams are up-to-date
    ...
    * But sometimes one sample is missing, I'm almost 100% sure that the generator is not perfect, simple sleep() call between samples
...
Timeline in processing time domain:
