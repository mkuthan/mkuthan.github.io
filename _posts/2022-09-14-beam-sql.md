...
Let's give it a try with [Apache Beam](https://beam.apache.org/) 😀
...
You will get an error because table does not exist yet, it will be created just before the first insert.
...
Unfortunately I could not manage deployment using `gcloud command`.
...
It does not make any sense to allocate job resources for the data received from the up-to-date sensors 
...
Why we have to pay for the whole message delivery if timestamp attribute is enough for tracking? I don't know.
{: .notice--info}
...
You could not change this behaviour.
{: .notice--info}
...
* Do not expect any order when Dataflow back-fills the results.
...
2. **Phase 2** both streams are up-to-date:
    ...
    * But sometimes one sample is missing. 
      Dataflow correctness is 100% but the generator is not perfect.
      Simple `sleep(1)` call between samples is not enough to generate exactly one event per second.
...
Even for the tiny throughput do not expect that Direct runner will be able to run the complete pipeline in the finite time.
